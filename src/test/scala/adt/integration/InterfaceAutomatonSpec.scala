...
/**
 * Tests the parsing and Dot Representation of a Single Graph
 * containing 3 nodes and edges between these note with metadata of different type with a common superclass.
 *
 * This test shows exemplary how to define an AST for the metadata types and construct a custom decoder for metadata
 * json content and how to use this decoder when parsing the json file.
 * Further, an edgeTransformer is constructed that shows how to customly transform edges into
 * a desired printed form containing HTML like strings.
 */
class InterfaceAutomatonSpec extends RefSpec with Matchers {
  object `testing the parsing and dot representation of an interface automaton` {
    ...
    object ast {
      ...
      sealed case class ReadVar(variable: Variable,
                                keyValRef:Int,
    ...
    for{
      ...
      for{topLevelGraphContainer <- jsonDecodingRes} yield {
        ...
        Option[(DotGraph,DotEdgeStmt)] = {
          ...
          val dot_sorted_without_WS = {
            val linesSeparated = dotRepresentation.dot.linesIterator.toList
            val linesSeparatedWithoutWS = linesSeparated.map(_.filterNot(_.isWhitespace))
            ...
            val sortedLines = linesSeparatedWithoutWS.sorted((s1:String, s2:String) => { (s1,s2) match {
              case ("digraph{",_) => -1
              case (_,"digraph{") => 1
              case ("}",_) => 1
              case (_,"}") => -1
          ...
          val expectedValueSortedWithoutWS = expectedValueSorted.filterNot(_.isWhitespace)

          expectedValueSortedWithoutWS should equal (dot_sorted_without_WS)
