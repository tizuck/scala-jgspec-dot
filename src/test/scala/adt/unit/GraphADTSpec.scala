...
/** Test cases for [[com.github.tizuck.jsonGraphSchema.graphDecoder]]. This
  ...
  * accepts an instance of [[io.circe.DecodingFailure]] and produces [[None]] as
  * a result we need to implement our own decoders that strictly fail when a
  * value can not be parsed. To be completely clear, the following examples
  ...
  * }}}
  *
  * If we use a Decoder for
  * [[com.github.tizuck.jsonGraphSchema.DirectedHyperGraph]] that uses standard
  * Optional Decoders then the field of hyperedges will fail to parse, but
  * because the Decoder will return [[None]], the graph will still be
  * interpreted as an instance of
  * [[com.github.tizuck.jsonGraphSchema.DirectedHyperGraph]] with an empty edge
  * field. Instead we want the decoder to fail and receive an instance of
  * [[com.github.tizuck.jsonGraphSchema.UndirectedHyperGraph]].
  *
  * Further if no edges or nodes are specified then the type of the graph is
  * ambiguous. We expect it here to be parsed as an instance of
  * [[com.github.tizuck.jsonGraphSchema.SimpleGraph]].
  */
class GraphADTSpec extends AnyWordSpec {
  ...
  "A graph parser" should {
    ...
    "parse a graph with minimal fields to be identified as a undirected hyper graph accordingly" in {
      ...
      val res =
        assertUndirectedHyperGraph[Unit, Unit, Unit](jsonParse, identity)
      // Result is not a parsing mistake
      assert(res.isRight)
  ...
  private def assertSimpleGraph[M1, M2, M3](
    ...
    for { p <- jsonParse } yield {
      ...
      for { res <- res } yield {
        assert(
          bf(res.isInstanceOf[SimpleGraph[M1, M2, M3]]),
          s"Instead type is: ${res.getClass}"
        )
      }
    }
  }

  private def assertHyperGraph[M1, M2, M3](
      jsonParse: Either[ParsingFailure, Json],
      bf: Boolean => Boolean
  )(implicit
      m1Dec: Decoder[M1],
    ...
    for { p <- jsonParse } yield {
      ...
      for { res <- res } yield {
        assert(
          bf(res.isInstanceOf[DirectedHyperGraph[M1, M2, M3]]),
          s"Instead type is: ${res.getClass}"
        )
      }
    }
  ...
  private def assertUndirectedHyperGraph[M1, M2, M3](
      jsonParse: Either[ParsingFailure, Json],
      bf: Boolean => Boolean
  )(implicit
      m1Dec: Decoder[M1],
    ...
    for { p <- jsonParse } yield {
      ...
      for { res <- res } yield {
        assert(
          bf(res.isInstanceOf[UndirectedHyperGraph[M1, M2, M3]]),
          s"Instead type is: ${res.getClass}"
        )
      }
    }
