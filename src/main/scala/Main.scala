                  ...
                  case Some(value) =>
                    value match {
                        ...
                        Table.empty
                          .appendRow(
                            Row(
                              List(
                                "type",
                                s"${EventSendVarId.getClass.toString}"
                              )
                            )
                          )
                          .appendRow(
                          ...
                          .appendRow(
                            Row(
                              List(
                                "op-ctx",
                                s"${operationCtx.transactionId},${operationCtx.spawnId}"
                              )
                            )
                          )
                          .toHTML
                      case ReadVar(
                        ...
                        Table.empty
                          .appendRow(
                            Row(List("type", s"${ReadVar.getClass.toString}"))
                          )
                          .appendRow(
                            Row(List("ia-name", s"${s.label.getOrElse("")};"))
                          )
                          .appendRow(
                            Row(
                              List(
                                "variable",
                                s"(${variable.name},${variable.ctx.transactionId},${variable.ctx.spawnId})"
                              )
                            )
                          )
                          ...
                          .appendRow(
                            Row(
                              List(
                                "result-var",
                                s"${resultVariable.name}, sId:${resultVariable.ctx.spawnId}, tId:${resultVariable.ctx.transactionId}"
                              )
                            )
                          )
                          .appendRow(
